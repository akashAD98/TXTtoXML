...
IMG_PATH = "D:/Testing/TestImgs"
fw = os.listdir(IMG_PATH)
# path of save xml file
save_path = '' # keep it blank

# txt_folder is txt file root that using darknet rectbox
txt_folder = 'D:/Testing/TestTxts'

# (NOTICE #3)
# Change this labels
labels = ['pistol','revolver','rifle','time bomb','tank','sniper','rocket launcher','dagger','sword','axe','artillery'
...
def extract_coor(txt_file, img_width, img_height):
    ...
    return label,x_min_rect, x_max_rect, y_min_rect, y_max_rect
...
for line in fw:
    ...
    for ii in range(len(txt_file)):
        ...
        x,x_min_rect, x_max_rect, y_min_rect, y_max_rect = extract_coor(
...
# Step 4b (If images have an extra dot now apply this script on the XML folder)
# path_to_directory='C:/Users/maind/Desktop/Newfolder/xmls'
# import os
# list_of_xmls=os.listdir(path_to_directory)
# #print(list_of_xmls)
# for i in list_of_xmls:
#     if i.count('.')>1:
#         x=i.replace('.', '', 1)
#         os.rename(path_to_directory+'/{}'.format(i), path_to_directory+'/{}'.format(x))
#         print(str(i)+"-> Converted to ->"+str(x))
